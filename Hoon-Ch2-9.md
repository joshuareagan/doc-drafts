...
sort: 9
next: true
title: Hoon Standard Library: Trees, Sets, and Maps
---

# Hoon Standard Library: Trees, Sets, and Maps
...
Here's a program that takes two sets of atoms and returns the Cartesian product of those sets.  A Cartesian of two sets `a` and `b` is a set of all the cells whose head is a member of `a` and whose tail is a member of `b`.
...
There are other map functions in the Hoon standard library we won't cover here.
